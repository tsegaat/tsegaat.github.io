...
const basic_info_type_profile_picture = document.getElementsByClassName(
	"basic-info-type-profile-picture"
)[0];
const basic_info_type_profile_picture_icon =
	document.getElementsByClassName("basic-info-types")[0];
const basic_info_type_profile_picture_input = document.getElementById(
	"basic-info-type-profile-picture-input"
);
...
firebase.auth().onAuthStateChanged(function (user) {
	if (user) {
		...
		ref.child("user_profile_pictures/" + photoid)
			.getDownloadURL()
			.then((url) => {
				basic_info_type_profile_picture.setAttribute(
					"style",
					`background-image: url(${url})`
				);
			})
...
basic_info_type_profile_picture_input.addEventListener(
	"change",
	handleManageProfilePicture,
	false
);
...
const basic_info_type_value_name = document.getElementsByClassName(
	"basic-info-type-value"
)[1];
	...
	if (user) {
		...
		dbf.collection("users")
			...
			.then((doc) => {
				const firstName = doc.data().firstName;
				const lastName = doc.data().lastName;
				basic_info_type_value_name.innerHTML =
					firstName + " " + lastName;
			});
	}
});
...
const personal_info_container = document.getElementsByClassName(
	"personal-info-container"
)[0];
const security_container = document.getElementsByClassName(
	"personal-info-container"
)[1];
...
auth.onAuthStateChanged(function (user) {
	if (user) {
		const basic_info_type_value1 = document.getElementsByClassName(
			"basic-info-type-value"
		)[5];
...
const basic_info_type_name =
	document.getElementsByClassName("basic-info-types")[1];
const basic_info_type_birthday =
	document.getElementsByClassName("basic-info-types")[2];
const basic_info_type_gender =
	document.getElementsByClassName("basic-info-types")[3];
const basic_info_type_password =
	document.getElementsByClassName("basic-info-types")[4];
const basic_info_type_security_password =
	document.getElementsByClassName("basic-info-types")[5];
const basic_info_type_security_2fa =
	document.getElementsByClassName("basic-info-types")[6];
const basic_info_type_contact_email = document.getElementById(
	"basic-info-type-contact-email"
);
const basic_info_type_contact_phone = document.getElementById(
	"basic-info-type-contact-phone"
);
...
basic_info_type_security_2fa.addEventListener("click", () => {
	...
	if (auth.currentUser.emailVerified) {
		...
		swal(
			"Oops...",
			"You must verify your email before trying to activate 2FA",
			"error"
		).then((value) => {
			if (value == true) {
				swal(
					"Email Verification",
					"Click the button below to get a verification link!",
					"info",
					{
						button: "Send link",
					}
